...
class BoundingLNode: public BoundingNode<BoxType>
{
 ...
 public:
    ...
    bool intersect_all(std::vector<float>&, const CGLA::Vec3f&,const CGLA::Vec3f&);
...
template<class BoxType>
inline bool BoundingLNode<BoxType>::intersect_all(std::vector<float>& pts, const CGLA::Vec3f& p,
                                                  const CGLA::Vec3f& dir)
{
#if USE_LEAF_BOXES
    assert(0);
