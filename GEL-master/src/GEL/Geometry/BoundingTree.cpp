...
template<class BoxType>
std::vector<float> BoundingTree<BoxType>::intersect_all(const CGLA::Vec3f& p,const CGLA::Vec3f& dir)
{
    std::vector<float> intersects;
    root->intersect_all(intersects, p, dir);
    
    // Remove identical intersection point
    double eps = 0.0001;
    std::vector<float> processed_intersects;
    for (auto t : intersects)
    {
        bool existed = false;
        for (auto t1 : processed_intersects)
        {
            if (abs(t-t1) < eps)
            {
                existed = true;
        ...
        if (!existed)
        {
            processed_intersects.push_back(t);
        }
    }
    
    // Sort intersection points
    std::sort(processed_intersects.begin(), processed_intersects.end());
    
    return processed_intersects;
}
